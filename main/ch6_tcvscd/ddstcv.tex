...
The last critical issue to address concerns the expected behavior of a data exchange involving the \texttt{DDSDataSource} within a MARTe2 application running on the SCD. Data exchanged over the reflective memory network can be grouped in two distinct classes, differentiated by their purpose:
\begin{itemize}
...
\end{itemize}
The perspective that better highlights this problem is that of a \texttt{RFM2g} DataSource that reads data from the shared memory area. Since the reflective memory has an internal clock, and because MARTe2 threads have to execute a real-time loop at a fixed rate, the \texttt{RFM2g} DataSource component can synchronize itself with the hardware and ensure that the thread rate is the same as the RFM clock. Since the RFM satisfies the timing requirements of magnetic control, this is feasible. However, when using not \texttt{RFM2g} but some other generic DataSource component, which relies on a different technology and whose cycle time may be subject to jitter (as is the case with, \emph{e.g.}, an Ethernet network even in the most ideal configuration), this behavior is not guaranteed and the DataSource may not be able to respect the real-time deadlines of the fast control loop. This might have critical and potentially destructive repercussions on the plasma equilibrium, as the control algorithms may not be able to actuate the coils in time to stabilize the plasma. To this end, even the internal clock of the SCD node that runs the real-time thread containing the DataSource is a more reliable source of timing information. The solution to this problem is to handle the two situations described above in the two different ways described below.
\begin{description}
  \item[\texttt{Synchronized}] The reader DataSource is synchronized with the communication channel clock and reads data at the highest rate allowed by it. This mode is used for data that is not involved in fast control, since the jitter may cause the DataSource to miss deadlines. This also implies that, if new data is produced at each clock cycle, the DataSource returns a new data sample at each iteration of the real-time thread loop.
  \item[\texttt{NotSynchronized}] The reader DataSource is not synchronized with the communication channel clock and reads data at a rate specified by some other component in the real-time thread (\emph{e.g.}, a \texttt{LinuxTimer} DataSource). This rate can be set equal to that of the communication channel clock but the DataSource is not bound to it, thus being allowed to deterministically meet fast control deadlines. A direct consequence of this behavior is that, if new data is produced at each clock cycle, the DataSource returns a new data sample only when the clock cycle is completed, else it returns again the last valid sample. Clearly this makes the DataSource act as a sort of sample-and-hold device but repeating a sample occasionally does not cause problems in the SCD control loop in practical scenarios and is preferred to missing deadlines.
\end{description}
To streamline the integration of the new \texttt{DDSDataSource} in the SCD, its behavior has to be exactly the same as described above at least for DataSources acting as \texttt{Subscribers}.
