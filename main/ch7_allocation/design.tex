...
\begin{assumption}
  \label{ass:lqrall_real}
  The closed-loop feedback interconnection of \eqref{eq:linplant} and \eqref{eq:lincontroller} does not induce pole-zero cancellations.
\end{assumption}

For $s\in\C$, let $\planttf(s)$ be the transfer matrix of the system in eq. \eqref{eq:linplant} from the input $\plantinput$ to the output $\plantoutput$ with $\ctrlouput=\bbaux=0$, and $\plantalltf(s)$ be the transfer matrix of the closed loop $\bbsys$ from the input $\bbaux$ to the output $\plantoutput$; moreover, let $(\bbdynmat,\bbctrmat,\bboutmat,0)$ be a realization of $\plantalltf(s)$. Thanks to \Cref{ass:lqrall_real}, the dimensions of $\planttf(s)$ and $\plantalltf(s)$ coincide with those of the realizations used to compute them.
...
\begin{problem}
	...
	Consider a continuous-time feedback control system $\bbsys$ described by \eqref{eq:linplant}, \eqref{eq:lincontroller}, and \eqref{eq:linexosystem}. Suppose that \Cref{ass:bbiss,ass:bboveract,ass:bbssresp,ass:lqrall_real,ass:lqrall_kernels} hold, and define $\bar{M}$ as in \eqref{eq:lqrall_memontinv_mbar}.
