...
Let $s$ be a variable taking values in $\C$, and $\planttf(s)$ be the transfer matrix of system \eqref{eq:linplant} from $\plantinput$ to $\plantoutput$ with $\bbaux=0$, computed from the state-space description of $\plant$. Let $\plantalltf(s)$ be the transfer matrix of \eqref{eq:bbsys} from $\bbaux$ to $\plantoutput$; moreover, let $(\bbdynmat,\bbctrmat,\bboutmat,0)$ be a realization of $\plantalltf(s)$. The dimensions of $\planttf(s)$ and $\plantalltf(s)$ are well determined and coincide with those of their respective state-space description thanks to \Cref{ass:lqrall_closedloop}.
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
where $\exodynmat\in\R^{\bbexostatedim \times \bbexostatedim}$ is a block-diagonal matrix defined as
\begin{subequations}
	...
	\end{align}
\end{subequations}
with $\bbexostate(0) = \bbexostate_0\in\R^{\bbexostatedim}$. The natural pulses $\{\omega_1,\dotsc,\omega_h\}$ in \eqref{eq:linexoblocks} are positive integer multiples of $2\pi/\bbperiod$, \emph{i.e.}, such that $\omega_i = k_i\frac{2\pi}{\bbperiod}$, $k_i\in\N$, $i=1,\dotsc,h$.
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
where $\plantlcfd(s)$, $\plantalllcfd(s)$, $\plantlcfn(s)$ and $\plantalllcfn(s)$ are polynomial matrices. The following assumption is introduced to simplify the discussion, although it implies that the controller \eqref{eq:lincontroller} cannot embed an internal model of exosystem \eqref{eq:linexosystem}.
\begin{assumption}
  \label{ass:lqrall_kernels}
  There exists a left coprime factorization \eqref{eq:lqrall_lcfs} such that the following holds:
	\begin{subequations}
...
\begin{subequations}
	...
	\end{equation}
\end{subequations}
which is termed the \emph{pseudo-steady-state output map}. Let $\ctrlssoutput(\cdot)$ denote the steady-state controller output. The following equality holds (see \Cref{app:moments}, as well as \cite{astolfi_model_2010,scarciotti_data-driven_2017} for a proof):
\begin{equation}
...
\begin{lemma}
	...
	\end{equation}
	where $\alpha,\beta\in\R^\bbauxdim$; let $\vartheta = [\alpha' \; \beta']'$ and
	\begin{equation}
		\label{eq:lqrall_psi}
	...
	\begin{subequations}
		\begin{align}
			...
			\label{eq:lqrall_psisys_bis}
			 & \vartheta = \Psi^\perp(\bar{\omega})z, \quad z\in\R^\nu,
		\end{align}
	\end{subequations}
...
\begin{proof}
	...
	Note that \eqref{eq:lqrall_finiteprb} is an unconstrained, finite-dimensional optimization problem completely equivalent to \Cref{prb:lqrall_optcont} in the setting defined by the statement of \Cref{lem:lqrall_solstruct}. Thus, the solution may be computed by solving the equation
	\begin{equation}
...
A straightforward corollary of the previous result follows from \eqref{eq:lqrall_optcoeffs}. It tells, in essence, that if $\zeta(\cdot)$ contains a single harmonic with zero amplitude, then the optimal $v$ with the same single harmonic has zero amplitude as well.
