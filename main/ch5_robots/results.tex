...
\begin{figure}[t!]
  \centering
  \includegraphics[width=.8\textwidth]{images/ch5/ldc21_aula.png}
...
\begin{table}[t]
  ...
  \begin{tabular}{|c|c|c|}
    ...
    \hline
  \end{tabular}
  \caption{Laboratory test RMSE (m) along each axis.}
  \label{tab:ldc21aula}
\end{table}
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=.8\textwidth]{images/ch5/ldc21_gara.png}
...
\begin{table}[t]
  ...
  \begin{tabular}{|c|c|c|}
    ...
    \hline
  \end{tabular}
  \caption{2021 Leonardo Drone Contest run RMSE (m) along each axis.}
  \label{tab:ldc21run}
\end{table}
...
Formally, let one suppose that $n\in\mathbb{N}$ sensors are available, each one producing data at a same rate $f_s\in\mathbb{R}_{>0}$. The incremental, filtered sample at time $k\in\mathbb{Z}_{\geq 0}$ corresponding to each sensor may then be denoted by $\Delta_{i,f}[k]$ for $i=1,\dotsc,n$. Each sensor also provides information about its functioning state, considered either on or off for simplicity and denoted by $s_i[k]\in\{0,1\}$ for $i=1,\dotsc,n$. Let $\bar{\Delta}_f[k] = [\Delta_{1,f}[k]',\dotsc,\Delta_{n,f}[k]']'$, $\bar{s}[k] = [s_1[k],\dotsc,s_n[k]]'$, and $\alpha_1,\dotsc,\alpha_n\in\mathbb{R}$ such that $\sum_i \alpha_i = 1$. The blending stage of the algorithm is summarized by the following equations:
\begin{subequations}
...
The software framework used to manage the experimental implementation in \cite{bianchi_efficient_2023} was a first, embryonic version of what has subsequently become the DUA framework presented in \Cref{ch:dua}. There were, in fact, a variety of software tools involved, ranging from middleware like, \emph{e.g.}, \ROS, to the implementation of the ORB-SLAM2 algorithm and the ZED SDK (see \Cref{sec:dua_modules_zed_drivers}). All of these software required its own dependencies, as well as specific setup steps that were slightly different on each of the two boards. For this reason, Docker was introduced as a tool to manage the software dependencies and setup steps, as well as to ensure a consistent and reproducible behavior across all the boards and the development PCs. This allowed developers to focus on the solution and implementation of the sensor fusion and blending algorithm, streamlining the experimental validation presented in the paper. Different Docker images were created for each of the different platforms, including development PCs, which integrated all the necessary dependencies and software. The techniques applied to the creation of these Docker images have led to the development of the base units of \texttt{dua-foundation}, introduced in \Cref{sec:dua_foundation}. Then, the rest of the framework followed.
