...
Formally, let one suppose that $n\in\mathbb{N}$ sensors are available, each one producing data at a same rate $f_s\in\mathbb{R}_{>0}$. The incremental, filtered sample at time $k\in\mathbb{Z}_{\geq 0}$ corresponding to each sensor may then be denoted by $\Delta_{i,f}[k]$ for $i=1,\dotsc,n$. Each sensor also provides information about its functioning state, considered either on or off for simplicity and denoted by $s_i[k]\in\{0,1\}$ for $i=1,\dotsc,n$. Let $\bar{\Delta}_f[k] = [\Delta_{1,f}[k]',\dotsc,\Delta_{n,f}[k]']'$, $\bar{s}[k] = [s_1[k],\dotsc,s_n[k]]'$, and $\alpha_1,\dotsc,\alpha_n\in\mathbb{R}$ such that $\sum_i \alpha_i = 1$. Let $\mathfrak{B}:\mathbb{R}^{2n}\times\mathbb{R}^n\rightarrow\mathbb{R}^2$ be the \emph{blending map} that produces the final increment sample. The blending stage of the algorithm is summarized by the following equations:
\begin{subequations}
