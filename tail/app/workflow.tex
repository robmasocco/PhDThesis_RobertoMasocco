\chapter{The dua-template update workflow}
\label{app:workflow}
\vspace{-1cm}

\begin{center}
\begin{lstlisting}[caption={\texttt{dua-template} update GitHub workflow as of November 22, 2024 \cite{masocco_dua-template_nodate}.}, label={lst:duawf}]
      ...
      - name: Push changes and create a pull request
        ...
        run: |
          if [[ -n $(git -C "${THIS_REPO}" status --porcelain) ]]; then
            ...
            git -C ${THIS_REPO} add .
            git -C ${THIS_REPO} commit -m \
            "chore: sync to dua-template@${{ github.sha }}"
            if [[ -n $(git -C ${THIS_REPO} branch -r --list \
            "origin/${HEAD_BRANCH}") ]]; then
              ...
              popd
              echo "INFO: Created new pull request from \
              ${HEAD_BRANCH} to ${BASE_BRANCH}"
            fi
          else
